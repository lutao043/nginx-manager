...
- **当前处于发布候选阶段**：G1–G6 已全部达标（见 ROADMAP 第三节），默认只做发布前验证与缺陷修复，不再新增功能；新的功能想法记进 ROADMAP 而不是直接写代码。
  - **例外（2026-09-22，用户指令）**：用户在发布前明确要求先加「日志实时跟随 + 可暂停」，该功能已落地（契约 `API.md` 的 `since/offset/reset/hasMore` → 后端增量读取 → 前端 1s 跟随轮询与暂停开关），并在 ROADMAP 第五节登记。**这类例外只在用户明确点名时成立**，加完仍回到「只修缺陷」口径，不得由此自行解冻。
