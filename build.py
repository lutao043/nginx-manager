...
def extract_version() -> str:
    """从 server.py 的 server_version 提取版本号（如 '0.3.1' 或 '9.9.9-rc.1'）。

    允许 `-` 预发布后缀：tag/产物名要能区分 rc 与正式版，后缀不参与资源版本数字段。
    """
    with open(SERVER_PY, encoding="utf-8") as f:
        m = re.search(r'server_version\s*=\s*["\']nginx-manager/([\d.]+(?:-[0-9A-Za-z.]+)?)["\']', f.read())
    if not m:
...
def version_parts(version: str) -> list:
    """把版本号拆成 Windows 资源版本的 4 段数字。

    Windows 资源版本只接受整数段，没有预发布位，故 `-rc.1` 这类后缀不参与编码，
    只取数字前缀并补齐 4 段；完整字符串（含后缀）写进 FileVersion/ProductVersion 文本属性。
    """
