...
- **配置文件管理**：树形浏览 + 在线编辑；保存前本地 diff 预览变更，保存时可选「保存并备份 / 仅保存」，随后执行 `nginx -t` 校验，校验失败一键回滚；校验错误可一键跳转到出错行
- **反向代理管理**：可视化切换 `proxy_pass` 目标（多备选、别名、按路径/地址搜索），改动直接写入 nginx.conf 并自动校验生效；内置场景模板（WebSocket / SSE 流式 / 大文件上传）一键套用
- **负载均衡 upstream**：可视化新建/编辑 upstream（轮询、least_conn、ip_hash，权重/备机/下线），代理目标填 `http://名称` 即接入
- **实时连接指标**：自动检测 stub_status 状态页，状态栏展示活跃连接与请求速率；未配置时一键开启（仅允许本机访问）
- **目标地址池**：全部 proxy_pass 目标统一管理；等价写法（大小写/默认端口/末尾斜杠）自动去重
- **日志实时跟随**：错误日志 + 访问日志自动滚动跟随，随时可暂停/继续；暂停期间新日志照常累积并提示未读行数，阅读位置不被打断；访问日志支持切换日志文件与本地关键字过滤
- **备份对比**：任意备份与当前文件之间 diff 对比
- **页面版本号 + 更新历史**：顶栏常显 manager 自身版本号，点开可逐版本查看更新说明（说明随 exe 一起打包，离线可读）。**不做自动更新**：不联网检查、不下载、不静默升级，升级 = 用新版 exe 替换旧 exe
